        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8 lg:gap-12 [perspective:1000px]">
          {PROJECTS_CONTENT.map((project, index) => (
            <motion.div
              ...
              className="h-full"
            >
              <TiltCard className="group relative flex flex-col h-full bg-white dark:bg-dark-card rounded-[2.5rem] overflow-hidden border border-slate-200 dark:border-dark-border hover:shadow-2xl hover:shadow-accent/5 transition-shadow duration-500">
                {/* Image Container */}
                ...
                <div className="flex flex-col flex-1 p-8 md:p-10" style={{ transformStyle: 'preserve-3d' }}>
                  <h3 
                    className="text-2xl md:text-3xl font-display font-bold text-slate-900 dark:text-white mb-4 group-hover:text-accent transition-colors"
                    style={{ transform: 'translateZ(35px)' }}
                  >
                    {project.title}
                  </h3>
                  <p 
                    className="text-slate-600 dark:text-slate-400 mb-8 line-clamp-3 leading-relaxed text-lg flex-1"
