...
const ProjectCard: React.FC<{ project: ProjectItem; index: number }> = ({ project, index }) => {
    ...
    return (
        <div 
            ref={cardRef}
            className={`group rounded-lg border border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-800/50 overflow-hidden transition-all duration-300 hover:shadow-2xl hover:shadow-rose-500/20 dark:hover:shadow-rose-400/20 hover:-translate-y-2 ${isVisible ? 'animate-fade-in-up' : 'opacity-0'}`}
        ...
        >
            <div className="relative w-full h-48 bg-slate-200 dark:bg-slate-700">
                ...
                <img 
                    ...
                    className={`w-full h-48 object-cover transition-opacity duration-500 ${imageLoaded ? 'opacity-100 group-hover:opacity-75' : 'opacity-0'}`} />
            </div>
            <div className="p-6">
                ...
                <div className="flex items-center gap-4 mt-auto">
                    {project.liveUrl && (
                        <a href={project.liveUrl} target="_blank" rel="noopener noreferrer" className="flex items-center gap-2 text-slate-500 dark:text-slate-400 hover:text-accent dark:hover:text-dark-accent transition-colors">
                            <IconExternalLink className="h-5 w-5" />
                            Live Demo
                        </a>
                    )}
                    {project.repoUrl && (
                         <a href={project.repoUrl} target="_blank" rel="noopener noreferrer" className="flex items-center gap-2 text-slate-500 dark:text-slate-400 hover:text-accent dark:hover:text-dark-accent transition-colors">
